...
var getFiles = function(cb, ctx){
  var p = path.resolve( process.cwd(), MIGRATION_FOLDER)
  fs.readdir(p , cb);
};
...
var readFiles = function(files){
  return function(cb, ctx){
    ...
    var processFile = function(filename, cb){
      var filePath = path.resolve(process.cwd(), MIGRATION_FOLDER, filename);
      async.waterfall([
...
var doInit = function(conString){
  async.auto({
    "connection": connection(conString),
    "createMigration":createMigrationFolder,
...
var doCreate = function(name){
  ...
  async.each(files, function(file, cb){
    var filePath = path.resolve(process.cwd(), MIGRATION_FOLDER, file.name);
